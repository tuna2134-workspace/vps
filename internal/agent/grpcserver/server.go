...
func (s *Server) GetConsoleToken(ctx context.Context, req *agentv1.GetConsoleTokenRequest) (*agentv1.GetConsoleTokenResponse, error) {
	if s.FakeMode {
		if req.GetConsoleType() == "serial" {
			return &agentv1.GetConsoleTokenResponse{SerialPath: "/dev/pts/0", Token: "fake-token"}, nil
		}
	...
	switch req.GetConsoleType() {
	case "serial":
		pty, err := s.libvirt.GetSerialInfo(vmName(req))
		if err != nil {
			return nil, status.Error(codes.NotFound, "serial console unavailable: "+err.Error())
		}
		return &agentv1.GetConsoleTokenResponse{SerialPath: pty}, nil
	default:
